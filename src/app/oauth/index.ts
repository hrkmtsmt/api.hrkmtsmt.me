...
import { Api, Cache } from "@modules";
import type { BlankSchema } from "hono/types";
import type { Env } from "@types";
import type { InitiateResponse } from "@modules/api/hatena/oauth.types";
...
export const oauth = new Hono<Env, BlankSchema, "/">()
	...
	.get("/oauth/hatena/callback", async (c) => {
		...
		const { oauth } = new Api(c.env).hatena;
		const t = await oauth.token(
			value.oauthToken,
			value.oauthTokenSecret,
			verifier,
		);
		const response = {
			ACCESS_TOKEN: t.oauthToken,
			ACCESS_TOKEN_SECRET: t.oauthTokenSecret,
		};
		cache.clear();
